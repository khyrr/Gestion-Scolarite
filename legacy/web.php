...
// use App\Http\Controllers\Legacy\ClasseController;
// use App\Http\Controllers\Legacy\CoursController;
// use App\Http\Controllers\Legacy\EnseignantController;
// use App\Http\Controllers\Legacy\EnseignPaiementController;
// use App\Http\Controllers\Legacy\EtudePaiementController;
// use App\Http\Controllers\Legacy\EtudiantController;
// use App\Http\Controllers\Legacy\EvaluationController;
// use App\Http\Controllers\Legacy\NoteController;
...
Route::prefix(config('admin.prefix'))->name('admin.')->middleware('admin.ip')->group(function () {
    ...
    Route::get('/dashboard', [App\Http\Controllers\Legacy\Admin\AdminDashboardController::class, 'index'])
        ->name('dashboard')
        ->middleware(['auth', 'role:admin', 'require.2fa:if_enabled']);

    // IP Whitelist Management
    Route::get('/settings/ip', [App\Http\Controllers\Legacy\Admin\AdminIpController::class, 'index'])
        ->name('settings.ip')
        ->middleware(['auth', 'role:super_admin']);

    Route::post('/settings/ip', [App\Http\Controllers\Legacy\Admin\AdminIpController::class, 'store'])
        ->name('settings.ip.store')
        ->middleware(['auth', 'role:super_admin', 'require.2fa']);

    Route::patch('/settings/ip/{ip}/toggle', [App\Http\Controllers\Legacy\Admin\AdminIpController::class, 'toggle'])
        ->name('settings.ip.toggle')
        ->middleware(['auth', 'role:super_admin', 'require.2fa']);

    Route::delete('/settings/ip/{ip}', [App\Http\Controllers\Legacy\Admin\AdminIpController::class, 'destroy'])
        ->name('settings.ip.destroy')
        ->middleware(['auth', 'role:super_admin', 'require.2fa']);

    // Activity log viewer / export
    Route::get('/logs', [App\Http\Controllers\Legacy\Admin\ActivityLogController::class, 'index'])->name('logs.index')->middleware(['auth', 'role:super_admin']);
    Route::get('/logs/export', [App\Http\Controllers\Legacy\Admin\ActivityLogController::class, 'export'])->name('logs.export')->middleware(['auth', 'role:super_admin', 'require.2fa']);
    ...
    Route::get('/2fa/setup', [App\Http\Controllers\Legacy\Admin\TwoFactorController::class, 'showSetup'])
        ->name('2fa.setup')
        ->middleware(['auth', 'role:admin']);

    Route::post('/2fa/enable', [App\Http\Controllers\Legacy\Admin\TwoFactorController::class, 'enable'])
        ->name('2fa.enable')
        ->middleware(['auth', 'role:admin']);

    // Regenerate secret (step-up required)
    Route::post('/2fa/regenerate', [App\Http\Controllers\Legacy\Admin\TwoFactorController::class, 'regenerate'])
        ->name('2fa.regenerate')
        ->middleware(['auth', 'role:admin']);

    // Only super_admins can disable 2FA for themselves (or others)
    Route::post('/2fa/disable', [App\Http\Controllers\Legacy\Admin\TwoFactorController::class, 'disable'])
        ->name('2fa.disable')
        ->middleware(['auth', 'role:super_admin']);

    Route::post('/2fa/clear-pending', [\App\Http\Controllers\Legacy\Admin\TwoFactorController::class, 'clearPending'])
    ...
    Route::middleware(['require.2fa.challenge'])->group(function () {
        Route::get('/2fa/challenge', [App\Http\Controllers\Legacy\Admin\TwoFactorController::class, 'challenge'])->name('2fa.challenge');
        Route::get('/2fa/recovery', [App\Http\Controllers\Legacy\Admin\TwoFactorController::class, 'recovery'])->name('2fa.recovery');
        Route::post('/2fa/verify', [App\Http\Controllers\Legacy\Admin\TwoFactorController::class, 'verify'])->name('2fa.verify');
    });

    // Admin Management
    // List and manage administrators (super admin only, 2FA required)
    Route::get('/admins', [App\Http\Controllers\Legacy\Admin\AdminManagementController::class, 'index'])
        ->name('admins.index')
        ->middleware(['auth', 'role:super_admin', 'require.2fa']);

    Route::post('/admins/{admin}/toggle-2fa', [App\Http\Controllers\Legacy\Admin\AdminManagementController::class, 'toggle2fa'])
        ->name('admins.toggle_2fa')
        ->middleware(['auth', 'role:super_admin', 'require.2fa']);

    Route::get('/admins/create', [App\Http\Controllers\Legacy\Admin\AdminManagementController::class, 'create'])->name('admins.create')->middleware(['auth', 'role:super_admin', 'require.2fa']);
    Route::post('/admins', [App\Http\Controllers\Legacy\Admin\AdminManagementController::class, 'store'])->name('admins.store')->middleware(['auth', 'role:admin']);
